...
只有 `scope=GLOBAL` 的全局插件目录分页会写入
`%LOCALAPPDATA%\FanVPNBridge\product-cache-v1`，最多保留 6 小时，并受 256 项、单项 8 MiB、内存
64 MiB 和磁盘容量上限约束。账号检查、已安装插件状态、模型响应、MCP、Cookie、Token 和用户内容
不会落盘。首次安装仍需真实同步一次目录，之后即使 Chrome 或 Native Host 重启也可命中。
