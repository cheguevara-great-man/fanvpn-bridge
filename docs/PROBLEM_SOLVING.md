...
### Chrome 关闭后 Bridge 断线及冷启动代理认证弹窗（2.6.3）

**现象**：关闭最后一个 Chrome 窗口后 VS Code 浏览器模式失去网络；重新启动 Chrome 时，服务器代理可能
在扩展恢复凭据之前弹出原生用户名密码框。

**根因**：原登录任务达到 ready 后立即退出，Chrome 后台模式也未被安装器显式启用。最后一个窗口关闭时，
Chrome、Offscreen Document、Native Messaging Host 和代理认证缓存可能一起消失。下一次冷启动恢复标签页
与代理扩展恢复凭据之间存在竞争窗口。

**解决**：安装器为当前用户启用 Chrome 官方后台模式，并把登录任务改为隐藏常驻看护；Bridge 连续两次
不可用时只执行 `--no-startup-window` 恢复。Browser Gateway 0.2.1 也在 service worker 每次冷启动时立即
恢复代理并预热认证，不再只等待 `runtime.onStartup`。

**经验**：浏览器执行链路必须有 Chrome 进程，但不要求存在可见窗口。保持同一 profile 的后台进程比在
每次可见窗口启动时重新竞争代理认证更稳定。
