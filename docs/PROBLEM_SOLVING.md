...
**Full 快速启动但 Apps 目录偶发 504（2.6.2）**：实测模型和 MCP 连续到达时，连接器目录主动让路
4 次后会在总期限之前提前失败；插件目录预热也可能在让路期间耗尽 15 秒。2.6.2 保留 15 秒硬期限，
但不再把主动让路次数本身当成失败，并把按账号隔离的连接器目录持久化一小时。模型/MCP 仍保持最高
优先级，目录不会反向阻塞聊天。
...
## WebHarness 压缩恢复后缺少 cwd

WebHarness 接入后出现过一个只在多轮、压缩或自动重试后暴露的问题：后续请求可能只重放旧轮次的
环境信封，而不再次携带当前 `cwd`。如果执行器只读取隔离后的 `CODEX_HOME`，会把原本已经验证过的
本地工作区误判为缺少可信环境，从而出现 `ChatGPT web turn is missing cwd in trusted Codex environment context`。

**解决**：安装配置继续写入执行器自己的 `integration`，但线程环境的只读校验查询真实 Codex home。
启动器在隔离 `CODEX_HOME` 前保存 `BRIDGE_WEB_CODEX_AUTHORITY_HOME`；运行时只把它用于历史 rollout 与
当前任务 visualization 根目录校验，不用于安装、登录或修改用户配置。后续请求可以省略初始环境，
执行器按线程 ID、当前 turn ID、工作区和权限校验原生记录后恢复；显式冲突或格式错误的环境更新仍拒绝。

Bridge 清理网页模型历史中的本地 reasoning 项时还必须保留消息 ID，否则会破坏原生环境来源标记。
模型目录刷新则合并已知原生模型缓存，避免模式切换或单次刷新失败删除已经发现的 GPT 模型。

**2026-09-09 验收**：失败的原生 Codex 会话在重启执行器后可恢复历史；独立测试会话连续完成
`apply_patch` 创建文件、终端读取、再次 `apply_patch` 修改并读取，只读测试会话仍拒绝写入。
同时通过 52 项环境/集成测试、28 项 Bridge/模型目录测试和 20 项启动器界面连线测试。

**经验**：压缩恢复不能靠“猜 cwd”兜底。可信目录必须来自同一线程已经验证过的原生环境记录，并且
恢复环境不能放宽原有沙箱或跨线程借用状态。

## WebHarness v5.0.6 合并与真实链路验收

完整合入上游 v5.0.6 后，Bridge 保留了网络配置、Codex 配置隔离和真实线程目录适配。

**2026-09-10 验收**：类型检查通过；184 项网页/环境/Bridge 测试与 14 项 Host 测试通过；75 项启动器
测试中 74 项通过，另 1 项受 Windows 文件符号链接权限限制。真实 Full/automatic 会话连续两轮文本回答
及第三轮只读 Get-Location 工具调用完成，没有出现 DOM 消失或已发送文本冲突。

当时仍未覆盖编辑重发等全部网页交互，CLI 的远程插件列表请求也仍出现连接错误。网页登录检查超时会使
升级事务回滚；应进入正常聊天页后重新完成设置，不能只手工修改 `releaseVersion` 绕过运行时校验。

**经验**：网页链路的本地测试只能证明适配层和已覆盖流程正常，不能把一次真实账号验收外推为所有模型、
插件、Connector 或企业网络环境都已验证。
