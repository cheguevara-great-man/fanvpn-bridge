...
**插件目录最后一页偶发超时（2.5.0）**：一次完整刷新连续成功读取 27 页，但最后一个带
`pageToken` 的 GET 在浏览器代理中超过 Codex 的 30 秒客户端超时，导致整个全局目录刷新被判失败。
这不是分页格式错误，而是只读元数据请求在弱网下缺少响应头超时、重试和跨入口复用。

**解决**：扩展只对固定 ChatGPT 插件元数据路径的幂等 GET 设置 10 秒响应头期限并最多重试一次，
总时间仍低于客户端超时。普通插件状态查询进入最多 3 并发的中优先级池，建议/精选查询保留第 4 个槽，
大体积的全局目录分页进入
单并发后台槽；账号初始化、模型、MCP 或工具请求到达时，可在插件 GET 响应头前抢占并让其随后续跑。
Host 对成功的 `scope=GLOBAL` 页面做
10 分钟、按账号及 Authorization 摘要隔离的有界内存缓存，让 18888 与 8000 两个受限入口共享，且把
相同页面的并发 miss 合并为一个上游请求。模型请求、MCP POST、工具调用、安装状态和任何含
`Set-Cookie` 的响应都不进入缓存，也不自动重试。
