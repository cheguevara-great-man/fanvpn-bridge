...
**第一次尝试（2.2.1，已撤回）**：新增固定上游 `chatgpt-backend` route，并让 Browser 模式把
`chatgpt_base_url` 指向该路由。真实电脑的日志显示多个产品后端请求约 21 秒后同时失败，客户端随后
连续重试，最终连正常的 `chatgpt-codex` 请求也受到拖累。把不同域名、不同认证语义和不同失败策略的
产品控制面整体塞进一个路由，并不能得到透明代理效果。

**当前解决（2.2.2）**：Browser 模式只保留已经验证稳定的模型数据通道，并临时设置
...
**后续方向**：保留通用路由和分发基础设施，按插件同步、Apps/MCP、账号信息和任务元数据等接口族
逐项捕获目标域名、请求头和响应协议；每组必须有独立超时、并发和失败隔离，验证后再加入 Browser
扩展模式，而不是重新启用整包 `chatgpt-backend`。
