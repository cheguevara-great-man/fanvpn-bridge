...
Bridge 3.0.0 可以把多台 Windows 电脑通过**浏览器精简**或**浏览器完整**链路产生的 Codex
...
- 当前 Codex Usage 窗口已经使用和剩余的百分比，以及精确重置时间；
- 在管理员明确设置的周期内，单台机器达到均分上限后阻止其后续模型请求。

它可以读取 Codex 客户端当前看到的官方**已用百分比、窗口和重置时间**，但该产品接口不提供
固定的 Credits 总额度，也不是公开稳定 API。因此官方数值仍以 Codex 的 `Settings > Usage`
页面为准；本项目显示的 Credits 是根据公开费率计算的可审计估算值。
...
Bridge 还会每 5 分钟读取一次 Codex 自己使用的 Usage 产品接口。只提取套餐类型、已用百分比、
窗口长度、重置时间和是否达到限制，然后上传脱敏快照；不会上传接口响应中的邮箱、用户 ID、
账号 ID、Cookie 或登录 Token。该产品接口没有公开稳定性承诺，因此同步失败只会让额度卡片标记为
过期，不会影响聊天。
...
Bridge 3.0.0 将推理档位和速度档位分别上报，避免旧版把两者混在同一个字段中。
...
- **总览**：每台设备的每日 Credits 折线图、统计期设备总额、官方窗口已用/剩余比例；
- **机器**：比较所有机器的缓存率、输入、输出、Credits 和主要模型；
- **机器详情**：单台机器的模型组合与每日历史；
- **模型**：模型、推理档位、标准/Fast、倍率、Token 构成、Credits 占比和标准费率；
- **设置**：仅管理员可设置周期预算和覆盖模型标准费率。

所有页面支持任意起止日期，最长 366 天。没有显式日期时，优先显示管理员设置的控制周期；
没有控制周期时使用最新官方 Usage 窗口；两者都没有时显示最近 30 天。

### 官方百分比与推算额度

官方 Usage 响应提供“已用百分比”，但不提供这个订阅窗口对应的固定 Credits 总数。仪表盘会用：

```text
推算总 Credits = 本项目在同一窗口记录的 Credits ÷ 官方已用比例
```

这个推算只有在所有相关用量都经过 Bridge、且从窗口开始就持续记录时才接近真实值。直连、Cloud
任务、其他 Agent 功能或活动重置都可能造成偏差，因此推算值只用于“谁可多用、谁应少用”的建议，
绝不自动触发硬停机。

### 六等分硬上限

管理员可在“设置”中填写起止日期、周期总 Credits、设备数（默认 6），并显式勾选硬上限。
每台上限等于 `周期总 Credits ÷ 设备数`。达到上限后，Bridge 在本机直接拒绝后续
`chatgpt-codex` 模型 POST，请求不会发往 OpenAI；登录、额度同步、历史和其他元数据仍可访问。

额度只能在模型任务完成后确定，所以最后一个任务可能使机器略微超过上限。中央服务器暂时不可达
时客户端沿用最近一次策略，初次从未取得策略时默认放行，避免服务器故障导致所有机器误停。
...
- 旧版 2.7/2.8 队列仍可上报，但只有 3.0.0 客户端会同步官方额度并执行硬上限；
