...
- `product_cache.py`：按账号和 Token 摘要隔离、有限容量、仅进程内存在的全局插件目录缓存。
...
Browser Full 对产品控制面另有两项稳态策略：MCP 客户端按 Streamable HTTP 协议发出的可选 GET
由 Host 本地返回 405，`.well-known` 探测也在本地结束，正式 MCP POST 仍原样转发；全局插件目录
`GET /backend-api/ps/plugins/list?scope=GLOBAL` 的成功 JSON 页面可在内存缓存 10 分钟。缓存键同时包含
账号与 Authorization 的 SHA-256 摘要，设置硬性条目、单页和总内存上限，不缓存 Token、Cookie、
安装状态、MCP 请求或模型响应，Host 退出后缓存立即消失。相同缓存键的并发 miss 会合并为一次上游
请求，18888 与 8000 的等待者共用结果。

扩展使用三级调度：账号初始化、模型、MCP 和工具请求最高；插件元数据居中，普通查询最多 3 个并发，
建议/精选查询可使用第 4 个保留槽；大体积、分页的全局插件目录最低且只有 1 个并发槽。更高优先级到达时，插件元数据 GET
可在响应头返回前安全让路并随后继续。只有固定 ChatGPT 主机和固定插件元数据路径的幂等 GET 可以做
响应头超时重试，任何 POST 都不会自动重发。
