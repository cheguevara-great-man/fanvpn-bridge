...
`chatgpt-codex` 承载登录后的 Codex 模型请求和模型目录。`chatgpt-backend` 承载 Codex 启动时读取的
Apps MCP、插件目录和产品元数据等 ChatGPT 后端请求；浏览器模式通过 Codex 官方
`chatgpt_base_url` 配置把这些请求送入 Bridge，避免本地进程绕过 Chrome 后等待网络超时。
`auth-openai` 承载 Token Exchange、刷新和注销；普通 API 请求不会进入该路由。
