...
### 为什么 Codex 显示压缩时，ChatGPT 网页仍可能继续回答

Codex 的自动压缩与 ChatGPT 网页生成不在同一个后端事务中。Codex 决定压缩后，WebHarness
会异步收到取消或交接通知，并尽力停止当前网页生成；该通知不能保证立即中断 ChatGPT
服务器端的推理。因此会存在以下时序竞争：

- 停止通知到达时网页仍在生成：WebHarness 会尝试停止生成；
- 停止通知生效前回答已经完成：不能再撤回该回答，WebHarness 应保存并复用完整终态答案；
- 工具调用仍在执行或结果尚未确定：不能把半截文字当作最终答案，也不能无条件重复执行任务；
- 压缩完成后 Codex 改写消息标识：WebHarness 会按稳定任务身份匹配并复用压缩前已经完成的答案，避免另开网页重复回答或重复修改文件。

这里的“停止”是尽力取消，不是同步终止保证。WebHarness 只能观察 ChatGPT 网页的停止按钮、
流式状态、回答文本和完成操作等 DOM 信号，无法直接确认 ChatGPT 后端内部是否仍在推理。
停止按钮消失只表示网页不再呈现生成状态，不等于已经正常完成。如果回答区域发生重绘、
文本骤减，或没有出现可信的完成操作，执行器应当报出网页终态识别错误，而不是擅自交付
残缺答案。

因此，压缩期间短暂看到原网页继续输出并不必然表示压缩失效。需要区分“通知尚未生效但
回答抢先完成”和“网页异常导致执行器无法确认终态”。诊断时应保留对应的
`diagnostics/browser-turns/<trace>` 目录，并对比生成状态、工具调用状态、最后稳定文本和完成
操作；不要仅凭 Codex 中的重连提示判断为模型拒绝或网络故障。
