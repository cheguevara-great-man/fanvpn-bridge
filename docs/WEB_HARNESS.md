...
- 当前 Bridge 发行版本：v5.0.6-bridge.2。Windows 安装包已内置并校验 OpenAI tunnel-client v0.0.12；目标电脑配置 MCP 时不再临时访问 GitHub 下载该文件。
...
需要同时更新本分支的 Chrome 扩展和 Native Host（3.10.4）。只替换扩展目录不能更新后端。
