...
Antigravity CLI 1.1.5 在模型列表下方新增了 `Effort` 强度滑杆，旧版社区扩展无法识别
这套新布局。CLI 实际正在隐藏终端中等待选择，因此侧栏看起来像卡死。项目内置的兼容版
会在侧栏显示可点击的模型选项；更新后重新执行“一键配置 Antigravity”并重开 VS Code。
当前强度沿用 CLI 中已选择的值，如需同时调整 `Effort`，可以点击聊天标题栏的终端按钮，
在镜像终端中使用左右方向键调整。

### 图片文件已经生成，但聊天中不显示

社区扩展原本只渲染文字和网页链接，既不支持 Markdown 图片，也不允许 Webview 直接读取
本地路径。兼容版会把当前 VS Code 工作区内实际存在的 PNG、JPEG、GIF、WebP 或 SVG
转换成受限的 Webview 资源地址并显示预览；工作区之外的文件不会开放给聊天页面。
更新后重新执行“一键配置 Antigravity”并完全重开 VS Code。为了自动显示，生成文件应位于
当前打开的工作区内。

### 点击登录后终端只打印 `agy-browser.exe` 路径

这表示旧版扩展把带引号的路径当成 PowerShell 字符串，而没有真正执行。再次运行“一键配置
Antigravity”并重开 VS Code。Windows 兼容版点击登录时会打开独立的 `PowerShell -NoProfile`
窗口并正确启动 CLI；登录成功后扩展会自动检测 Windows 凭据并刷新聊天界面，不需要再点击
“无需登录”或手工创建标记文件。
