...
> **普通用户推荐从这里开始。** 本文只保留第一次初始化和日常升级的最短路径。
> 需要手动构建、Native Host 注册细节、A/B 回滚、日志或卸载时，再看
> [Windows 完整安装参考](INSTALLATION.md)。
...
默认保留原安装目录。只有需要迁移目录时才填写目录输入框，并填入目标项目文件夹的完整绝对路径，例如 `D:\AI\my-bridge`。文件夹名称不再要求以 `fanvpn-bridge` 或 `browser-gateway` 结尾；目标可以是尚不存在的文件夹、空文件夹或已经能够识别为对应项目的目录，但不能是磁盘根目录或无关的非空目录。迁移后的第一次，Chrome 必须由你在 `chrome://extensions` 中对新目录执行一次“加载已解压的扩展程序”；以后该新目录也支持一键更新。
