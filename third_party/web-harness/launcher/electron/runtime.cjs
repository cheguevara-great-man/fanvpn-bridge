...
class RuntimeHost {
  ...
  captureSetupCheckpoint(snapshot) {
    ...
    const paths = new Set([
      ...
      path.join(process.env.BRIDGE_WEB_MANAGED === "1" ? path.join(coreHome, "integration") : this.codexHome, "config.toml"),
      path.join(process.env.BRIDGE_WEB_MANAGED === "1" ? path.join(coreHome, "integration") : this.codexHome, "models_cache.json"),
