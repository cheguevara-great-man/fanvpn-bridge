...
export function compileChatGptWebPrompt(
  ...
  const transportContract = parsed._compactionRequest
    ...
    ? [
      ...
      "Do not dump entire logs, session transcripts, databases, or response-state JSON into the conversation. Extract selected fields and matching records with bounded output. If a result was truncated, refine the query instead of repeating the same broad read.",
      "After a checkpoint, continue from its verified findings and next action. Re-read previously inspected material only to resolve a specific missing fact or verify changed state; do not restart the investigation from the beginning.",
